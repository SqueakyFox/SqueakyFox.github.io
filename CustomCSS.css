...
/* Custom Status */
.theme-dark .root-1gCeng, .theme-dark .footer-3rDWdC {
    background: var(--background-overlay);
}
...
.reactionMe-23mbRf:hover .reactionCount-2ddRoS {
...
.theme-dark .button-2CgfFz, .theme-dark .button-2CgfFz:hover, .theme-dark .card-FDVird:before { /* also affects game activity list */
    background-color: var(--background-overlay);
}
...
/* Server Boost in Server Settings*/
...
.theme-dark .tierInProgress-3mBoXq {
    background-color: var(--background-overlay);
    border: 0.1em solid rgb(244,127,255);
}
...
.theme-light .sampleLink-KPFu3I, .regionSelect-3lf4eE button {
    color: var(--main-color);
}
...
/* hey did you know you can fuck with the upload file thing? */
...
/* Links in textarea (legacy at first) */
.fakeLink-11RWCQ, .fakeLink-26oLcc {
    color: var(--main-color);
}

/* Blocked user "chat box" */
.wrapper-39oAo3 {
    background-color: var(--background-overlay);
}
...
/* server unavalible screen. bit of an easter egg. */
...
/* Games tab */
.scroller-1IIF0A, .scroller-2XE8rp {
...
/* merge this with the other shit once it's on stable. */
/* upload modal popup background */
...
/* used in BOTH new message markers and date*/
.divider-JfaTT5 {
...
/* chat bar - has the nifty side effect of being broken in 2 weeks. */
.channelTextArea-rNsIhG {
    background-color: transparent;
}
/* this is ONLY to fix a slight clipping from chat*/
.form-2fGMdU {
    padding-top: 8px;
}
/* invite */
...
/* Allow background - "Playing" list */
.outer-2IVh5n {
    background-color: transparent;
}
/* Shading of left side */
.peopleList-3c4jOR {
    background-color: rgba(0, 0, 0, 0.4);
}
/* Thin and shade left side */
.nowPlayingColumn-2sl4cE {
    background-color: var(--background-overlay);
    min-width: 240px;
}
/* "Now Playing" user background */
.wrapper-Lii66c{
...
/* "This section could be better" box. Disable something in the privacy settings to see it. */
.consentCard-1iSxB_,
/* No friends playing games? Not sure. */
.emptyCard-1RJw8n
{
    background-color: var(--background-overlay);
}
...
/* "Now Playing" user background, hover */
